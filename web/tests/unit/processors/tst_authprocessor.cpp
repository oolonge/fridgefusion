...
public:
    ...
    std::optional<User> getUserByEmail(const QString& email) override {
        if (email == "test@example.com") {
            ...
            return User(2, "admin", "admin@example.com", "admin_password_hash", QDateTime::currentDateTime(), UserRole::ADMIN);
        }
        return std::nullopt;
    }
    ...
    QList<User> getUsersByUsername(const QString& username) override {
        ...
        if (username.contains("admin")) {
            users.append(User(2, "admin", "admin@example.com", "admin_password_hash", QDateTime::currentDateTime(), UserRole::ADMIN));
        }
        return users;
    }
    ...
    bool verifyCredentials(const QString& email, const QString& passwordHash) override {
        ...
        if (email == "admin@example.com" && passwordHash == "admin_password_hash") {
            return true;
        }
        return false;
    }
...
public:
    QString hashPassword(const QString& password) override {
        // For testing, we just append "_hashed" to simulate hashing
        return password + "_hashed";
    }

    bool verifyPassword(const QString& password, const QString& hash) override {
        // For testing, we simulate password verification
